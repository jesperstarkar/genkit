...
describe('GoogleCloudLogs with json', () => {
  ...
  it('writes expected Input log attributes', async () => {
    ...
    logObjects.forEach((logBlob) => {
      ...
      // generate request
      if (logBlob.message.startsWith('Input[generate > testModel, generate]')) {
  ...
  it('writes expected Output log attributes', async () => {
    ...
    logObjects.forEach((logBlob) => {
      ...
      if (
        logBlob.message.startsWith('Output[generate > testModel, generate]')
      ) {
        expect(logBlob.model).toEqual('testModel');
  ...
  it('writes expected Path log attributes', async () => {
    ...
    logObjects.forEach((logBlob) => {
      if (logBlob.message.startsWith('Path[')) {
        expect(logBlob.flowName).toEqual('testFlow');
        expect(logBlob.paths).not.toBeUndefined();
        expect(logBlob.paths).toHaveLength(3);
      }
    });
  });
